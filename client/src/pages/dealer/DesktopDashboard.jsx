 ...
 */
// Door type descriptions shown to dealer so they know what to pick
const DOOR_TYPE_INFO = {
    'PVC':  { emoji: '🚪', desc: 'Lightweight, waterproof. Best for bathrooms & kitchens.' },
    'WPC':  { emoji: '🪵', desc: 'Wood-plastic composite. Stronger, premium finish.' },
    'HPL':  { emoji: '✨', desc: 'High Pressure Laminate. Scratch & heat resistant.' },
};
...
export default function DesktopDealerDashboard({
    ...
    posts,
    getImageUrl,
    searchQuery, setSearchQuery, handleReorder,
    showOrderConfirm, lastPlacedOrder, dismissOrderConfirm
}) {
    ...
    if (showOrderConfirm && lastPlacedOrder) {
        return (
            <div className="flex h-screen bg-gray-100 font-sans overflow-hidden items-center justify-center">
                <div className="bg-white rounded-3xl shadow-2xl p-12 max-w-md w-full text-center animate-in fade-in zoom-in-95 duration-300">
                    ...
                    <div className="bg-red-50 rounded-2xl p-4 my-6 text-left space-y-2">
                        <div className="flex justify-between">
                            <span className="text-sm font-bold text-gray-500">Order Number</span>
                            <span className="text-sm font-black text-red-700">#{lastPlacedOrder.id}</span>
                        </div>
                        <div className="flex justify-between">
                            <span className="text-sm font-bold text-gray-500">Items</span>
                            <span className="text-sm font-black text-gray-900">{lastPlacedOrder.itemCount} item(s)</span>
                        </div>
                        <div className="flex justify-between">
                            <span className="text-sm font-bold text-gray-500">Status</span>
                            <span className="text-sm font-black text-orange-600">RECEIVED — In Queue</span>
                        </div>
                    </div>
                    <p className="text-xs text-gray-400 mb-6">You will be notified on WhatsApp when your order is ready for pickup.</p>
                    <button
                        onClick={dismissOrderConfirm}
                        className="w-full bg-red-600 hover:bg-red-700 text-white font-black py-4 rounded-2xl transition-all shadow-lg shadow-red-200 text-lg"
                    >
                        View My Orders
                    </button>
                </div>
    ...
    return (
        <div className="flex h-screen bg-gray-100 font-sans overflow-hidden">
            <Sidebar
                ...
                isMobile={false} // Desktop Dashboard is always desktop view
                user={user}
                roleLabel="Dealer Portal"
                menuItems={menuItems}
                activeTab={activeTab}
                                        ...
                                        {!orderSelection.doorTypeId ? (
                                            ...
                                            <div className="animate-in fade-in slide-in-from-top-4">
                                                ...
                                                <div className="grid grid-cols-3 gap-3">
                                                    {doors.map(d => {
                                                        const info = DOOR_TYPE_INFO[d.name.toUpperCase()] || { emoji: '🚪', desc: d.thickness || '' };
                                                        return (
                                                            <button
                                                                key={d.id}
                                                                onClick={() => setOrderSelection({ doorTypeId: d.id, designId: '', colorId: '' })}
                                                                className="p-4 rounded-xl text-left bg-white hover:bg-red-50 shadow-sm border border-gray-100 hover:border-red-300 transition-all group"
                                                            >
                                                                <span className="text-3xl block mb-2 group-hover:scale-110 transition-transform">{info.emoji}</span>
                                                                <div className="font-black text-gray-900 text-sm">{d.name}</div>
                                                                <div className="text-[11px] text-gray-400 mt-1 leading-tight">{info.desc}</div>
                                                                <div className="text-[10px] font-bold text-red-400 mt-2 uppercase tracking-wider">{d.thickness}</div>
                                                            </button>
                                                        );
                                                    })}
                                                </div>
                                            ...
                                            // COLLAPSED Step 1
                                            <div onClick={() => setOrderSelection({ ...orderSelection, doorTypeId: '', designId: '', colorId: '' })} className="flex items-center gap-4 bg-red-50 border border-red-100 p-3 rounded-xl cursor-pointer hover:bg-red-100 transition-all">
                                                <div className="bg-red-600 text-white w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm">1</div>
                                                <div className="flex-1">
                                                    <div className="text-xs text-red-400 font-bold uppercase tracking-wider">Material</div>
                                                    <div className="font-black text-red-900">{doors.find(d => d.id == orderSelection.doorTypeId)?.name}</div>
                                                </div>
                                                <div className="text-red-400 text-xs font-bold uppercase px-3">Change</div>
                                            </div>
                                        )}
                                    </section>
                                    ...
                                    {orderSelection.doorTypeId && (
                                        <section className="mb-4">
                                            {!orderSelection.designId ? (
                                                    ...
                                                    <div className="flex justify-between items-center mb-3">
                                                        ...
                                                        <div className="relative">
                                                            ...
                                                            <input
                                                                ...
                                                                className="pl-7 pr-3 py-1.5 text-xs font-bold border rounded-lg outline-none focus:ring-2 focus:ring-red-500 w-40"
                                                                value={searchQuery}
                                                    ...
                                                    <div className="grid grid-cols-5 md:grid-cols-6 gap-3 max-h-[400px] overflow-y-auto custom-scrollbar p-1">
                                                        {filteredDesigns.map(d => (
                                                            <div
                                                                ...
                                                                className="rounded-xl overflow-hidden cursor-pointer shadow-sm hover:shadow-md transition-all ring-1 ring-gray-100 hover:ring-red-300 relative group"
                                                            >
                                                ...
                                                // COLLAPSED Step 2
                                                <div onClick={() => setOrderSelection({ ...orderSelection, designId: '', colorId: '' })} className="flex items-center gap-4 bg-red-50 border border-red-100 p-3 rounded-xl cursor-pointer hover:bg-red-100 transition-all">
                                                    <div className="bg-red-600 text-white w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm">2</div>
                                                    ...
                                                    <div className="flex-1">
                                                        <div className="text-xs text-red-400 font-bold uppercase tracking-wider">Design</div>
                                                        <div className="font-black text-red-900">{designs.find(d => d.id == orderSelection.designId)?.designNumber}</div>
                                                    </div>
                                                    <div className="text-red-400 text-xs font-bold uppercase px-3">Change</div>
                                                </div>
                                            )}
                                    ...
                                    {orderSelection.designId && (
                                        <section className="mb-6">
                                            {!orderSelection.colorId ? (
                                                    ...
                                                    <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                                                        <div className="flex flex-wrap gap-4">
                                                            {allColors.map(c => (
                                                                ...
                                                                >
                                                                    <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-transparent group-hover:border-red-400 group-hover:scale-105 transition-all shadow-sm">
                                                                        {c.imageUrl ? (
                                                                    ...
                                                                    <span className="text-[10px] font-bold mt-1 text-center text-gray-600 line-clamp-1 group-hover:text-red-600">
                                                                        {c.name}
                                                                    </span>
                                                                </div>
                                                            ))}
                                                ...
                                                // COLLAPSED Step 3
                                                <div onClick={() => setOrderSelection({ ...orderSelection, colorId: '' })} className="flex items-center gap-4 bg-red-50 border border-red-100 p-3 rounded-xl cursor-pointer hover:bg-red-100 transition-all">
                                                    <div className="bg-red-600 text-white w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm">3</div>
                                                    ...
                                                    <div className="flex-1">
                                                        <div className="text-xs text-red-400 font-bold uppercase tracking-wider">Color</div>
                                                        <div className="font-black text-red-900">{allColors.find(c => c.id == orderSelection.colorId)?.name}</div>
                                                    </div>
                                                    <div className="text-red-400 text-xs font-bold uppercase px-3">Change</div>
                                                </div>
                                            )}
                                        ...
                                        <section className="animate-in fade-in duration-300">
                                            <div className="flex justify-between items-center mb-4">
                                                ...
                                                <div className="flex gap-2">
                                                    <button onClick={() => addBulkRows(5)} className="text-xs font-bold text-red-600 bg-red-50 px-3 py-2 rounded-lg hover:bg-red-100 transition-all">
                                                        +5 Rows
                                                    </button>
                                                    <button onClick={() => addBulkRows(10)} className="text-xs font-bold text-red-600 bg-red-50 px-3 py-2 rounded-lg hover:bg-red-100 transition-all">
                                                        +10 Rows
                                                    </button>
                                                    <button onClick={addRow} className="text-sm font-bold text-white bg-red-600 px-4 py-2 rounded-xl flex items-center gap-2 hover:bg-red-700 transition-all shadow-md">
                                            ...
                                            <div className="bg-white p-4 rounded-2xl shadow-md border border-gray-100">
                                                ...
                                                <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                                                    {sizeRows.map(row => (
                                                        <div key={row.id} className="grid grid-cols-12 gap-2 items-center bg-gray-50/50 hover:bg-white p-2 rounded-xl border border-transparent hover:border-gray-200 transition-all group">
                                                            <input type="number" placeholder="W" className="col-span-3 bg-white border border-gray-200 rounded-lg p-2 text-center font-bold focus:ring-2 focus:ring-red-500 outline-none text-sm" value={row.width} onChange={e => updateRow(row.id, 'width', e.target.value)} />
                                                            <input type="number" placeholder="H" className="col-span-3 bg-white border border-gray-200 rounded-lg p-2 text-center font-bold focus:ring-2 focus:ring-red-500 outline-none text-sm" value={row.height} onChange={e => updateRow(row.id, 'height', e.target.value)} />
                                                            ...
                                                            <button
                                                                ...
                                                                className={`col-span-1 flex justify-center items-center h-9 rounded-lg transition-all ${row.hasVent ? 'bg-red-100 text-red-600 border border-red-200' : 'bg-gray-100 text-gray-300 hover:bg-gray-200'}`}
                                                        ...
                                                        */}
                                                            <div className="col-span-1 relative group/remark">
                                                                <input
                                                                    type="text"
                                                                    className={`w-full h-9 rounded-lg border text-xs px-2 outline-none focus:ring-2 focus:ring-red-500 transition-all ${row.remarks ? 'bg-yellow-50 border-yellow-200' : 'bg-white border-gray-200 text-transparent focus:text-gray-900 placeholder-transparent focus:placeholder-gray-400'}`}
                                                                    placeholder="Note"
                                                                    value={row.remarks}
                                                            ...
                                                            <input type="number" min="1" className="col-span-2 bg-white border border-gray-200 rounded-lg p-2 text-center font-bold focus:ring-2 focus:ring-red-500 outline-none text-sm" value={row.quantity} onChange={e => updateRow(row.id, 'quantity', Math.max(1, parseInt(e.target.value) || 1))} />
                                                ...
                                                <div className="mt-4 pt-4 border-t border-gray-100 flex justify-between items-center">
                                                    ...
                                                    <button onClick={addAllToCart} className="bg-red-600 hover:bg-red-700 text-white font-bold py-3 px-8 rounded-xl shadow-lg flex items-center gap-2 transition-all active:scale-95">
                            ...
                            <div className="w-96 bg-white border-l shadow-xl p-6 overflow-y-auto">
                                <div className="sticky top-0 bg-white pb-4 border-b mb-4">
                                    <div className="flex items-center justify-between">
                                        <h2 className="text-xl font-black flex items-center gap-2">
                                            <ShoppingCart className="text-red-600" /> Cart
                                        </h2>
                                        {cart.length > 0 && (
                                ...
                                {cart.length === 0 ? (
                                    ...
                                    <>
                                        ...
                                        <div className="mb-3">
                                            <label className="text-[11px] font-black text-gray-500 uppercase tracking-widest ml-1">Site / Party Name (for your reference)</label>
                                            <input
                                                type="text"
                                                value={siteName}
                                                onChange={(e) => setSiteName(e.target.value)}
                                                placeholder="e.g. 172, Mohandeep Society, Ishvarbhai"
                                                className="w-full mt-1 bg-gray-50 border border-gray-200 rounded-xl p-3 text-sm font-bold text-gray-700 focus:ring-4 ring-red-50 focus:border-red-300 transition-all"
                                            />
                                            <p className="text-[10px] text-gray-400 mt-1 ml-1">Helps you identify this order at delivery time</p>
                                        </div>
                                                ...
                                                <div key={item.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-shadow">
                                                    <div className="flex justify-between items-start mb-4">
                                                        <div>
                                                            ...
                                                            {item.siteName && <div className="text-sm font-black text-gray-900 mb-0.5">📍 {item.siteName}</div>}
                                                            <div className="text-sm text-gray-500">{new Date(item.createdAt).toLocaleDateString()}</div>
                                                            {item.expectedDate && ['RECEIVED', 'PRODUCTION', 'READY'].includes(item.status) && (
                                                                <div className="text-xs font-bold text-emerald-600 mt-1 flex items-center gap-1">
                                                                    📅 Expected ready: {new Date(item.expectedDate).toLocaleDateString(undefined, { day: 'numeric', month: 'short' })}
                                                                </div>
                                                            )}
                                                        </div>
                                                        ...
                                                        <div className="grid grid-cols-4 gap-2 mb-4">
                                                            {statusSteps.map((step, idx) => (
                                                                <div key={step} className="text-center">
                                                                    <div className={`h-2 rounded-full ${idx <= currentStep ? 'bg-red-500' : 'bg-gray-200'}`} />
                                                    ...
                                                    <div className="flex gap-2 mb-4">
                                                        ...
                                                        <button onClick={() => handleReorder(item)} className="flex-1 bg-red-50 hover:bg-red-100 text-red-600 py-2 rounded-lg text-sm font-bold transition-all flex items-center justify-center gap-2">
                                                        ...
                                                        <div className="flex items-center gap-4">
                                                            ...
                                                            <div>
                                                                ...
                                                                <div className="flex gap-2 mt-1">
                                                                    <span className="bg-red-50 text-red-700 px-2 py-0.5 rounded text-xs font-bold">{item.ordersCount} Orders</span>
                                                                    <span className="text-gray-400 text-xs flex items-center">Last: {new Date(item.lastDate).toLocaleDateString()}</span>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div className="text-right">
                                                            <div className="text-3xl font-black text-red-600">{item.totalItems}</div>
                                                            <div className="text-xs font-bold text-red-400 uppercase tracking-wider">Total Units</div>
                                                        </div>
                                                    </div>
                                                </div>
                                            );
